...
// Robust conversion from std::wstring to std::string using WideCharToMultiByte
...
class ArgumentDebuggerWindow {
 public:
  ...
  [[nodiscard]] bool Initialize(HINSTANCE h_instance, int cmd_show, const std::vector<std::wstring>& args);
  int RunMessageLoop();
 ...
 private:
  [[nodiscard]] HRESULT InitializeWindow(HINSTANCE h_instance, int cmd_show);
  [[nodiscard]] HRESULT InitializeDevice();
  [[nodiscard]] HRESULT CreateDeviceAndSwapChain(UINT width, UINT height);
  [[nodiscard]] HRESULT CreateRenderTargetView();
  [[nodiscard]] HRESULT CreateD2DResources();
  [[nodiscard]] HRESULT CreateShadersAndGeometry();
  void RenderFrame();
  void Cleanup();
...
int WINAPI wWinMain(HINSTANCE h_instance, HINSTANCE, PWSTR, int cmd_show) {
  ...
  if (!debugger_app.Initialize(h_instance, cmd_show, args))
    return 0;
...
[[nodiscard]] bool ArgumentDebuggerWindow::Initialize(HINSTANCE h_instance, int cmd_show, const std::vector<std::wstring>& args) {
  args_ = args;
  if (FAILED(InitializeWindow(h_instance, cmd_show)))
    return false;
...
[[nodiscard]] HRESULT ArgumentDebuggerWindow::InitializeWindow(HINSTANCE h_instance, int cmd_show) {
...
[[nodiscard]] HRESULT ArgumentDebuggerWindow::InitializeDevice() {
  RECT rc;
  GetClientRect(window_handle_, &rc);
  UINT width  = rc.right - rc.left;
  UINT height = rc.bottom - rc.top;

  HRESULT hr = CreateDeviceAndSwapChain(width, height);
  if (FAILED(hr))
    return hr;
  ...
  hr = CreateShadersAndGeometry();
  return hr;
}

[[nodiscard]] HRESULT ArgumentDebuggerWindow::CreateDeviceAndSwapChain(UINT width, UINT height) {
  ...
  return D3D11CreateDeviceAndSwapChain(
      ...
      d3d_device_.GetAddressOf(), &feature_level, immediate_context_.GetAddressOf());
}

[[nodiscard]] HRESULT ArgumentDebuggerWindow::CreateRenderTargetView() {
  ComPtr<ID3D11Texture2D> back_buffer;
  HRESULT hr = swap_chain_->GetBuffer(0, __uuidof(ID3D11Texture2D), reinterpret_cast<void**>(back_buffer.GetAddressOf()));
  if (FAILED(hr))
    return hr;

  hr = d3d_device_->CreateRenderTargetView(back_buffer.Get(), nullptr, d3d_render_target_view_.GetAddressOf());
  return hr;
}

[[nodiscard]] HRESULT ArgumentDebuggerWindow::CreateD2DResources() {
  // Create the Direct2D factory
  D2D1_FACTORY_OPTIONS options = {};
  HRESULT hr = D2D1CreateFactory(D2D1_FACTORY_TYPE_SINGLE_THREADED, options, d2d_factory_.GetAddressOf());
  if (FAILED(hr))
    return hr;
  ...
  hr = d2d_factory_->CreateDxgiSurfaceRenderTarget(dxgi_surface.Get(), &props, d2d_render_target_.GetAddressOf());
  if (FAILED(hr))
    return hr;
...
[[nodiscard]] HRESULT ArgumentDebuggerWindow::CreateShadersAndGeometry() {
  ...
  HRESULT hr = D3DCompile(vs_src, sizeof(vs_src) - 1, nullptr, nullptr, nullptr,
  ...
  hr = D3DCompile(ps_src, sizeof(ps_src) - 1, nullptr, nullptr, nullptr,
  ...
  hr = d3d_device_->CreateVertexShader(vs_blob->GetBufferPointer(), vs_blob->GetBufferSize(), nullptr, vertex_shader_.GetAddressOf());
  if (FAILED(hr))
    return hr;

  hr = d3d_device_->CreatePixelShader(ps_blob->GetBufferPointer(), ps_blob->GetBufferSize(), nullptr, pixel_shader_.GetAddressOf());
  if (FAILED(hr))
    return hr;
  ...
  hr = d3d_device_->CreateInputLayout(layout_desc.data(), static_cast<UINT>(layout_desc.size()),
  ...
  // Define vertices using std::array
  ...
  hr = d3d_device_->CreateBuffer(&bd, &init_data, vertex_buffer_.GetAddressOf());
  if (FAILED(hr))
    return hr;

  // Define indices using std::array (36 indices)
  ...
  hr = d3d_device_->CreateBuffer(&bd, &init_data, index_buffer_.GetAddressOf());
  if (FAILED(hr))
    return hr;
  ...
  bd.CPUAccessFlags = 0;
  hr = d3d_device_->CreateBuffer(&bd, nullptr, constant_buffer_.GetAddressOf());
  return hr;
}

// UpdateQrCode generates a QR code with the data "t=<unix_timestamp>;f=<fps>"
// and creates a bitmap of size 375×375 pixels.
// If command-line arguments exist, they are appended after an "args=" field.
// The QR code is updated no more than once every 5 seconds.
void ArgumentDebuggerWindow::UpdateQrCode(ULONGLONG current_time) {
  ...
  // Release previous bitmap automatically via ComPtr by resetting
  qr_bitmap_.Reset();

  HRESULT hr = d2d_render_target_->CreateBitmap(
      D2D1::SizeU(pixel_size, pixel_size),
      pixels.data(),
      pixel_size * sizeof(uint32_t),
      &bitmapProperties,
      qr_bitmap_.GetAddressOf());
  // (Error logging can be added if needed.)
}
...
void ArgumentDebuggerWindow::RenderFrame() {
  ...
  // Begin 2D drawing using Direct2D
  ...
  for (const auto& line : kDescriptionLines) {
    ...
    d2d_render_target_->DrawTextW(line.c_str(), static_cast<UINT32>(line.size()), text_format_.Get(), rect, white_brush.Get());
    y_pos += kLineHeight;
  }
  y_pos += kLineHeight;
  ...
  {
    D2D1_RECT_F rect = D2D1::RectF(kMargin, y_pos, size.width - kMargin, y_pos + kLineHeight);
    d2d_render_target_->DrawTextW(cli_text.c_str(), static_cast<UINT32>(cli_text.size()), text_format_.Get(), rect, green_brush.Get());
  }
  y_pos += kLineHeight;
  ...
  if (!args_.empty()) {
    ...
    d2d_render_target_->DrawTextW(joined_args.c_str(), static_cast<UINT32>(joined_args.size()), text_format_.Get(), args_rect, green_brush.Get());
    y_pos += kLineHeight;
  }
  ...
  {
    D2D1_RECT_F rect = D2D1::RectF(kMargin, y_pos, size.width - kMargin, y_pos + kLineHeight);
    d2d_render_target_->DrawTextW(count_str.c_str(), static_cast<UINT32>(count_str.size()), text_format_.Get(), rect, green_brush.Get());
  }
  ...
  d2d_render_target_->DrawTextW(exit_prompt.c_str(), static_cast<UINT32>(exit_prompt.size()), text_format_.Get(), exit_prompt_rect, yellow_brush.Get());

  D2D1_RECT_F user_input_rect = D2D1::RectF(kMargin, size.height - 60.0f, size.width - kMargin, size.height - 30.0f);
  d2d_render_target_->DrawTextW(user_input_.c_str(), static_cast<UINT32>(user_input_.size()), text_format_.Get(), user_input_rect, green_brush.Get());
...
void ArgumentDebuggerWindow::Cleanup() {
  // Reset all ComPtr objects; they will automatically release their resources.
